...
class ScanningTest extends FunSpec with ShouldMatchers with XMLMatchers {
  describe("Scanning Info") {
    it("should show 1 total pages when there are 1 total pages") {
      val html = <span id="total-pages">here</span>
      val job = Job("A Job", 1, Batch("A Batch"), Scanning(Nil))
    ...
    it("should show 145 total pages when there are 145 total pages") {
      val html = <span id="total-pages">here</span>
      val job = Job("A Job", 145, Batch("A Batch"), Scanning(Nil))
    ...
    it("should show 0 and 0% scanned pages when there are 0 scanned and 200 total pages") {
      ...
      val job = Job("A Job", 200, Batch("A Batch"), Scanning(Nil))
...
trait XMLMatchers  {
  ...
  def like(a: NodeSeq, b: NodeSeq) = {
    if(a xml_== b) None
    else Some(a + " is not like " + b)
  }
}
