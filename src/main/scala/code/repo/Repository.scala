...
object RepositoryInjector extends Factory {
  val jobRepository = new FactoryMaker[JobRepository](buildJobRepository _) {}
  
  def buildJobRepository: JobRepository = new SQLJobRepository
}
...
class FailingJobRepository extends JobRepository {
  ...
  override def getByMachineName(name: String) = Failure("Weird error when getting " + name, Full(new IOException()), Empty)  
...
class DumbJobRepository extends JobRepository {
  ...
  override def getByMachineName(name: String) = Full(
    Job("Australian Baptist 1925", 628, Batch(""), Nil)
      .add(ScanningProducerEvent(new DateTime, new Period(1, 30, 0, 0), 43))
  )
